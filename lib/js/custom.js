...
$( document ).ready(function() {
	...
	var checkNav=false;
	if(pathName=="/tong-hop"|| pathName=="/parttime" || pathName=="/fulltime" ||pathName=="/one-time"||pathName=="/tim-kiem.php") { $(".navbar").addClass('bg-primary'); checkNav = true;}
	else $(".navbar").addClass('nav-bg-white');
	...
	$(window).scroll(function(){
		...
		if (scroll > 20) {
			...
			if(checkNav) $(".navbar").addClass('bg-primary'); else $(".navbar").removeClass('bg-white');
		}
	});
...
$(".nav-item .card-header").click(function() {
	...
	fadeSearchFrom('#searchFormNav');
});
$("#job-list-wrap .card-header").click(function() {
	$(this).parent().find('.card-body').slideToggle(300);
});
//Handle click outside windown close search bar
$('#job-list-wrap').click(function(){
	$('#searchFormNav').hide();
	fadeSearchFrom('#job-list-wrap');
});

function fadeSearchFrom (idElement){
	$(this).find(idElement+' .icon-rotate').find('.fas').toggleClass('animate-icon');
};
// show tooltip
$(document).ready(function(){
